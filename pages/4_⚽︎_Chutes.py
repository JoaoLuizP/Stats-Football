...
if 'df_2' not in st.session_state or 'df_3' not in st.session_state:
    ...
    if st.session_state["time_a"] != None and st.session_state["time_b"] != None:
        ...
        with tab1:
            ...
            col2.metric(f"Média de chutes ao gol feitos", round(((df_filtered2['ShotsOnTarget_H'].sum())/len(df_filtered2)),2))
            col3.metric(f"Média de chutes ao gol sofridos", round(((df_filtered2['ShotsOnTarget_A'].sum())/len(df_filtered2)),2))
            col3_1.metric(f"Média de chutes feitos geral", round(((df_filtered2['Shots_H'].sum())/len(df_filtered2)),2))
